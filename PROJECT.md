...
- **Waterfall heads.** A *source* with air below that is fed (water above or beside it) does not fall: it stays put and pours flow down through the gap each pulse, so a falling stream is a persistent column that drops one cell per pulse. A source with no water around it (a lone block in the sky) does fall and lands as flow. Flow that lands on an existing pool rests on its surface and spreads sideways, so an open basin fed from a high head climbs layer by layer to the head's level.
