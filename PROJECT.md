...
- Water tries to flow **down** one cell per pulse first (a cell falls at most one level per pulse), and **any fall resets it to full level 7** — so water always prefers to run down a slope: a spring on a hillside fans out a few blocks, then runs off down to the next ledge instead of drowning the whole contour. At rest it depends on what is *below*: **on solid ground** it becomes a floor sheet that spreads to Air horizontally with **bounded range** — each sideways step costs one level (7→6→…→1) and a level-1 cell spreads nothing, so a spring's flood is a fan ~6 blocks out (rounded by the 4-way spread); levels are render-cosmetic (a cell always draws full height). **On water** it splits: over a cell that is itself a *stream*, or a shallow sheet (water one deep over solid) it stays and becomes a **stream** cell (`wstream` — visible, never spreads, starves with the body it is part of); over anything deeper (a deep pool, the sea) it **vanishes** — a waterfall pouring into the sea is simply lost. Consequently **no water body's level ever rises**: an open basin fed from a high head gets a floor sheet, a sealed basin holds only what its spring pours in, and caves under the ocean take a stream plus a floor pool — they do not fill.
- **Waterfall heads.** A *placed spring* with air below never falls: it is re-checked every slow-clock pulse and keeps pouring flow down through the gap, so a falling stream is a persistent column that drops one level per pulse — a spring in a cave wall or alone in the sky is an eternal emitter until the player removes it. A fed *static* source pours the same way — the sea column above a breached floor drops into the cave and becomes the cave's stream. A static source with no water around it (a lone drip) does fall and lands as flow, which starves once isolated.
