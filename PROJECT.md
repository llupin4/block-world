...
**Flow.** A cellular automaton over a 0–7 level and per-cell flags, stored in parallel `Uint8Array`s (`wlevel` is a constant 7 for every live water cell and does not render — the `source`/`sustained`/`placed`/`stream` flags do the work):

- There are **two kinds of source**. *Placed* water (only what the player places, `wplaced`) is a **spring**: level 7, immortal, and it keeps pouring flow into air beside it even at rest. *Worldgen* water (the ocean) is a **static** source: settle re-seeds it as a source so falls are handled in one bulk pass per chunk load, but it **never spreads or pushes** — the sea does not flood caves through a breach, and a high placed spring cannot raise the sea level. (Placing water in the sea does nothing to the sea.)
- Water tries to flow **down** one cell per pulse first (a cell falls at most one level per pulse). At rest it depends on what is *below*: **on solid ground** it becomes a floor sheet, spreading to Air horizontally with unlimited range — terrain and reachability, not levels, bound the water. **On water** it splits: over a cell that is itself a *stream*, or a shallow sheet (water one deep over solid) it stays and becomes a **stream** cell (`wstream` — visible, never spreads, starves with the body it is part of); over anything deeper (a deep pool, the sea) it **vanishes** — a waterfall pouring into the sea is simply lost. Consequently **no water body's level ever rises**: an open basin fed from a high head gets a floor sheet, a sealed basin holds only what its spring pours in, and caves under the ocean take a stream plus a floor pool — they do not fill.
- **Waterfall heads.** A *source* with air below that is fed (water above or beside it) does not fall: it stays put and pours flow down through the gap each pulse, so a falling stream is a persistent column that drops one level per pulse. A static worldgen source pours the same way — the sea column above a breached floor drops into the cave and becomes the cave's stream. A source with no water around it (a lone block in the sky) does fall and lands as flow, which starves once isolated.
