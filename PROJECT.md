...
> also drives a **deer** (a grazing quadruped with a sim-owned wander AI — `MobController`,
...
**Flow.** A cellular automaton over a 0–7 level and per-cell flags, stored in parallel `Uint8Array`s (`wlevel` is a real number — 7 at every fresh start, decaying to 1 with each sideways step and resetting to 7 on any fall — and it renders: a resting flow cell's surface sits at `wlevel / 8` of the cell (source and stream cells draw full; a taller cell skirts down against a lower water neighbour, `emitWater` in `src/chunk-mesher.ts`); the `source`/`placed`/`stream` flags decide full vs graded height):
