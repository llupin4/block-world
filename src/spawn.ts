...
/** A deer can spawn on this cell: grass with two air cells above, at a plausible surface height.
...
export function isDeerSpawnCell(getBlock: (x: number, y: number, z: number) => number, x: number, y: number, z: number): boolean {
  return getBlock(x, y, z) === Block.Grass && getBlock(x, y + 1, z) === Block.Air && getBlock(x, y + 2, z) === Block.Air && y >= -20 && y <= 48;
}

/**
 * Roll up to `count` deer spawn positions in chunk (cx,cz), drawing randoms from `next` (the
 * caller's PRNG — deterministic when seeded). For each random (x,z) column, find the SURFACE
 * (the topmost non-air cell) and land a deer on its top (y+1) only if that surface is a valid
 * grass spawn. Keying off the surface — not the highest grass cell — prevents landing on a grass
 * cave floor when the real surface above is dirt/stone (which buried deer and teleported the
 * player underground on possession). Skips columns that already host a deer (the `exists`
 * predicate). Returns positions (not entities) so the caller owns the spawn.
 */
export function rollDeerSpawns(
  ...
  for (let t = 0; t < 400 && out.length < count; t++) {
    ...
    let y = 48; // find the surface: the topmost non-air cell in the column
    while (y >= -20 && get(x, y, z) === Block.Air) y--;
    if (y >= -20 && isDeerSpawnCell(get, x, y, z) && !exists(x, y + 1, z)) out.push({ x: x + 0.5, y: y + 1, z: z + 0.5 });
  }
  return out;
}

/**
 * Spawn up to `count` deer into the sim for chunk (cx,cz), using a PRNG seeded from the chunk
...
export function spawnDeer(world: World, sim: Sim, cx: number, cz: number, count = 2): void {
  ...
  const exists = (x: number, y: number, z: number) =>
    sim.all().some((e) => e.kind.id === 'deer' && Math.round(e.pos.x) === x && Math.round(e.pos.z) === z);
  const rng = new SimRng(cx * 73856093 ^ cz * 19349663);
  for (const p of rollDeerSpawns(world, cx, cz, count, () => rng.next(), exists)) {
    const mc = new MobController(get, () => sim.rng.next());
    sim.spawn(p, mc, { kindId: 'deer', baseController: mc });
  }
}