# Spec: mobs, possession, spectator — one dolt, drive any entity, a spectator ghost
...
Add one mob — a grazing quadruped, the **dolt** — plus **possession** (drive any entity
...
tick-ordered** (never `Math.random`) so phase 3's replays stay deterministic. The dolt is
...
2. A box-part rig renders the dolt (head, body, 4 legs) and the player (head, body, 2
   legs) from the same system; the viewed entity's own rig is hidden in first person.
3. **Mob controller determinism:** a fixed seed drives a deterministic dolt path over 1200
   ticks; the dolt refuses to step off a ≥ 3-block drop or into water; it turns when it is
   blocked for N ticks.
...
- **`dolt`** — the grazing quadruped. `half 0.45`, `height 0.9`, `eye 0.7`, `walkSpeed
  1.6`, `swimSpeed 1.0`, `jumpVel 8.0` (clears one block; apex `8²/56 ≈ 1.14 m`), `flySpeed 0`, `flyVSpeed 0`,
  `canFly false`, `canNoclip false`, `canEdit false`, `collides true`. The dolt cannot edit —
...
`EntityRecord.controllerKind` gains `'mob'` (a restored dolt reattaches a `MobController`).
`controllerKindOf` recognizes `MobController` → `'mob'`.

## MobController (wander AI, `src/entity.ts`)

One `MobController` per dolt. A small state machine; **every draw is from the sim's PRNG**,
...
fact that each dolt draws a number of randoms that depends only on its own (deterministic)
...
- **blocked** — if forward progress is below a threshold for M consecutive ticks (M pinned,
   ...
   **any** tick with low forward progress — including a step the refusal rules just blocked
   (water / a ≥3 drop) — so a dolt facing water or a cliff turns within M ticks rather than
...
- At boot and on each **streaming load** of a new chunk column: roll for up to K dolts in
  that chunk, under a **global population cap** within the view ring (track the live dolt
count; stop spawning once at the cap — default **6**). A **spawn cell** is a grass surface
   ...
   height, `y >= 3`) with the dolt placed standing on it (feet at the grass top). Spawn only
   for **rebuilt** chunk columns (freshly generated, no saved record); **restored** chunks
   already carry their persisted dolts and are **not** re-rolled (re-rolling would double-
   populate them). All spawn rolls draw from `sim.rng` (deterministic). `[POC shortcut]`
- **Despawn:** a dolt despawns only when its chunk **unloads** (it persists with the chunk via
  the phase 1 entity-ride; it is restored on walk-back). A dolt that walks out of the ring
...
- `buildEntityRig(kind)` returns a parent `Object3D` holding a small set of `Mesh` boxes
  ...
  pivot: [x,y,z] (leg/hip pivot, else 0) }`. The dolt gets head + body + 4 legs; the player
...
- The **viewed entity's own rig is hidden** while in first person (the camera is its eye).
  ...
  morph targets, ≤ 6 draws per dolt.
...
- **`P` (`KeyP`)** handler: cast the **entity-pick** ray (nearest entity AABB before the
   ...
   - **hits an entity `t`:** possess it. The **previous viewed** entity's controller is set
     ...
     = `t`. `baseController` is set at spawn to the entity's "home" controller: a dolt's is
   ...
   - **no target, not at the body (possessing a dolt, or at the ghost):** return to the body
   ...
   `setViewed`).
- **Capabilities are the kind's, not the controller's.** Possessing the dolt means slow
  walk, one-block hop, no break, 0.7 m eye.
- **HUD:** a small label shows the viewed kind id (`player`/`dolt`/`spectator`); when the
  viewed entity `canEdit` is false the **hotbar is hidden** (the dolt/spectator can't place).
...
still flows through `applyIntent`; every kinematic change through `stepEntity`. The dolt is a
fully ordinary entity: spawned, stepped, persisted, and possessed exactly like the player.

## Pinned numbers (must not regress)

`dolt`: `half 0.45`, `height 0.9`, `eye 0.7`, `walkSpeed 1.6`, `swimSpeed 1.0`,
...
- Health, hunger, damage, tools, crafting. The dolt cannot be hurt yet.
...
- A dolt rides the phase 1 **edited-only** chunk gate: a dolt frozen in an **unedited**
  chunk is not persisted and vanishes on reload (the same as an unedited chunk's water).
  A dolt in an edited chunk persists with it. `[POC shortcut]`