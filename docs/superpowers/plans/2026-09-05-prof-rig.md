...
## R2 execution notes (2026-09-05, applied in place during Task 3)

The first green e2e run verified the rig end-to-end but exposed two errors in this plan's
code blocks; both are corrected in the committed implementation (`3b0e6d2`) and the embedded
blocks below are updated to match. The spec carries the full rationale as revision R2.

1. **Settle semantics (R2a).** `chunk.lightSettled` is live **only in the worker's mirror** —
   `applyLightResult` (`src/light-transport.ts`) applies blight/skylight + `touched` + queue
   state but does not carry the flag to main-thread chunks, so the main-thread flag is never
   true and the original settle condition (`wc.lightSettled && …`) deadlocks until the 600-
   frame cap. The rig instead settles on the **settled-light signature + quiescence**: a
   completed `sliced` cycle with `verts === 6312`, `PROF_QUIESCE` (10) remesh-free frames
   after the last worst-chunk remesh, and no remesh of the chunk pending or in flight
   (`!pendingRebuild.has && !scheduler.has`). A later touch restarts the quiescence (the
   self-correcting contract). `FrameState.worstSettled` drops the `lightSettled` term.
2. **Cycle arithmetic (R2b).** A 4-band split is **4 frames**, not 5: the plan frame meshes
   band 0; the in-flight advances are slice (band 1), slice (band 2), and the merge frame
   (meshes band 3, then `finish()` returns the merged geometry). Live run: two 6312-vert
   4-frame cycles at frames 193–196 and 205–208 (the second = a touched-during-split re-
   mesh). The ADR template and the unit tests are updated accordingly.
3. **Vitest scoping.** `tests/e2e/remesh-prof.spec.ts` matches vitest's default `*.spec.ts`
   include and would run under `npm test`; `vitest.config.mts` scopes the unit suite to
   `src/__tests__/**/*.test.ts`, and `test-results/`/`playwright-report/` are gitignored.

First green run (headless, render on): `window [108, 208]`, `settledVerts 6312`,
`maxWindowMs 8.3`, cycles 193–196 / 205–208 (maxFrameMs 4.3 / 4.5), ocean `max 7.8 / avg
2.2`, global max 52.6 ms on frame 0 (one-time shader compile, outside the window),
`drainMaxMs 15.8`, `pass: true`.
...
export interface FrameState {
  ...
  worstSettled: boolean; // !pendingRebuild.has(key) && !scheduler.has(key) — the chunk's lightSettled
                        // flag is live only in the worker mirror (the reply does not carry it),
                        // so the rig settles on quiescence + the 6312 baseline signature instead
}
...
- **Measured browser acceptance (2026-09-05, branch `slice-heavy-remesh`):** the deterministic
  ...
  **PASS**. Worst chunk `2,1,0` window = frames [W0, W1]; the settled remesh took the slice
  path (plan + 2 slice + merge = 4 frames over 4 bands; settledVerts = 6312 = node baseline); worst remesh frame
...
- **Frame-count arithmetic** (verified against the `finished` latch + begin/note alternation): happy path — window [60, 73], B frames 84..383 = 300, report at frame 383 (R2: 4-frame cycle + 10-frame quiescence shift the transition from frame 74 to frame 84); cap — transition at frame 599, B = 300 frames, report emitted.
